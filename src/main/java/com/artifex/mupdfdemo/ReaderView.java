...
import com.example.admin.tiff.R;
...
public class ReaderView
	...
	private static final float MAX_SCALE        = 8.0f;//最大缩放
	...
	private final SparseArray<View>
				  mChildViews = new SparseArray<View>(3);
					       // Shadows the children of the adapter view 阴影的适配器视图的孩子
					       // but with more sensible indexing
	private final LinkedList<View>
				  mViewCache = new LinkedList<View>();
	private boolean           mUserInteracting;  // Whether the user is interacting 用户是否交互
	private boolean           mScaling;    // Whether the user is currently pinch zooming 用户当前是否正在缩放
	private float             mScale     = 1.0f;
	private int               mXScroll;    // Scroll amounts recorded from events. 从事件记录的滚动量。
	...
	public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX,
		...
		if (v != null) {
			...
			//如果页面被拖出边界，那么我们希望能很好地返回页面。他立刻跳回了界内，
			// 所以我们不想在这种情况下使用掷球。另一方面，我们不想放弃一个投掷，
			// 只是因为稍微偏离角度的阻力把我们带出了阻力之外的方向，所以我们只在行进的方向上测试出界。
			//
	...
	public boolean onScaleBegin(ScaleGestureDetector detector) {
		...
		//忽略任何滚动量，但要注意：屏幕没有显示它们的效果，所以只能迷惑用户。
		mXScroll = mYScroll = 0;
		...
		if ((event.getAction() & MotionEvent.ACTION_MASK) == MotionEvent.ACTION_UP) {
			...
			if (v != null) {
				if (mScroller.isFinished()) {
					...
					//如果在用户交互结束时，操作中没有当前的惯性滚动条，那么在必要时将视图激活到屏幕上。
					slideViewOntoScreen(v);
				}
